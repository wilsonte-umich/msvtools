...
#------------------------------------------------------------------------
    train => [
        ...
        'method' =>         [1, 1,  "M", "<str>", "method used to construct model (bam|M|F)", "Input Options"],
        'max-mem'=>         [0, 2,  @maxMem],
        'tmp-dir'=>         [0, 3,  @tmpDir],
        'bin-size'=>        [0, 11, "z", "<int>", "use this bin size for paired-end bam method [5000]", "BAM Options"],
        'max-TLen'=>        [0, 12, "L", "<int>", "use this bin size for paired-end bam method [1000]"],
        'min-qual'=>        [0, 13, "q", "<int>", "minimum MAPQ of one read for a pair to be considered [5]"],        
		'reference'=>       [0, 14, "r", "<str>", "copy number reference region as chrom:start-end/copy_number"],       
		'max-copy-number'=> [0, 15, "c", "<int>", "maximum copy number allowed in the HMM [4]"],
        'chromosomes'=>     [1, 21, @chroms, "Output Options"],     
        'output-dir'=>      [1, 31, @output],
        'model-name'=>      [1, 32, @mdlName],
    }],
    refine => [ # NB: can repeat refine sequentially to establish convergence
		...
		'model-dir'=>       [0, 1,  "D", "<str>", "directory containing training model files", "Model Options"], 
        ...
        'samples' =>        [1, 14, "s", "<chr>", "comma-delimited list of samples to use from project"],
