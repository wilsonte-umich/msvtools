...
# adjacent runs of probes within a threshold are merged as complex events
...
# determine runs of contiguous anomalous probes
# allow for, but keep track of, closely adjacent but not abutting runs
message("finding runs of anomalous probes")
dd     <- d [anom,]
ddi    <- di[anom] # row numbers from d for rows present in dd (NOT row numbers from dd)
segN1_ <- 0 # sub-run identifier (may merge into run of adjacent runs)
segN2_ <- 0 # output run identifier
...
lrg_gap    <- c(TRUE, dd[nxt,'POS'] - dd[prv,'POS'] > MAX_GAP_BP)
dd$segN1  <- sapply( diff_chrom | any_gap,
    function(inc){ # break on new chromosome, or _any_ probe gap (sub-runs)
        if(inc) segN1_ <<- segN1_ + 1
        segN1_
    })
dd$segN2  <- sapply( diff_chrom | (any_gap & lrg_gap),
    function(inc){ # break on new chromosome, or _large_ probe gap (final output runs)
        if(inc) segN2_ <<- segN2_ + 1
        segN2_
    })
rm(prv, nxt, diff_chrom, any_gap, lrg_gap)

# filter away trivially short probe runs
# NB: only anomalous probes included in probe count and used for likelihood calculation
...
SVs <- data.frame(segN2=sort(unique(dd$segN2)))
by  <- list(segN2=dd$segN2)
SVs$N_PROBES <- aggregate(1:nrow(dd), by, function(x){
    length(which( # again, any probes in gap region are NOT in dd...
...
SVs[,c('MDL_LL','SV_LL')] <- t(sapply(1:nrow(SVs), function(i){
    ddi <- which(dd$segN2 %in% SVs[i,'segN2']) # row numbers from dd
...
# filter away probe runs without insufficiently greater likelihood relative to input model
message(paste("limiting output to runs with relative log likelihood >=", MIN_REL_LL))
keep_SVs(SVs$REL_LL >= MIN_REL_LL)

# determine the number of sub-runs fused into a single output run
message("aggregating SV parameters")
COUNT <- function(x) length(unique(x))
SVs$N_SUB_RUNS <- aggregate(dd$segN1, by, COUNT)[[2]]
...
flt <- d$IS_NA==0 & d$CN_IN==d$CN_OUT
CN_CLC <- Vectorize(function(LRR_ADJ) 2 * 2**LRR_ADJ)
CN_corr <- sapply(1:max(d$CN_IN,na.rm==TRUE), function(CN) {
    median(CN_CLC(d[flt & d$CN_IN==CN,'LRR_ADJ']))
})

SVs$CN_CLC <- aggregate(dd$LRR_ADJ, by, function(LRR_ADJ){
    sapply(sort(unique(CNs)), function(CN){
