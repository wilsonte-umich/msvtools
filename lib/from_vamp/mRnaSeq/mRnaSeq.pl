...
defined $command{createDatabase_Mrna} or $command{createDatabase_Mrna} = ['singleThread', '24:00:00', 20000, 0];
defined $command{createBowtieIndex_Mrna} or $command{createBowtieIndex_Mrna} = ['singleThread', '24:00:00', 5000, 0];
...
sub createDatabase_Mrna {
    my ($refSeqBase, $refSeq) = @_;
    my $refMrnaPath = getRefMrnaPath($refSeqBase, $refSeq);
    my $refSeqFile = "$refMrnaPath.fa";
...
sub createBowtieIndex_Mrna {
    my ($refSeqBase, $refSeq) = @_;
    my $refMrnaPath = getRefMrnaPath($refSeqBase, $refSeq);
    my $bowtieCommand = "$param{bowtiePath}bowtie-build -f $refMrnaPath.fa $refMrnaPath"; 
    runBowtie($bowtieCommand);    
}
sub getRefMrnaPath {
    my ($refSeqBase, $refSeq) = @_;
    $refSeqBase or $refSeqBase = $param{refSeqBase};
    $refSeq or $refSeq = "refMrna_$param{refSeq}";
    my $refMrnaPath = "$param{refSeqPath}/$refSeqBase/$refSeq";
    return $refMrnaPath;
}
...
sub addMrnaMaps { 
    ...
    #may eventually want to have way of mapping ONLY RNA data, but this easily could be derived from here
    ($inSample) = @_;
...
sub mapReadMrnaSeq_pass {
    ...
    my $refSeqFile = "$param{refSeqPath}/$param{refSeq}/$refSeq.fa"; #refSeq = refMrna_$param{refSeq}
    my $refdb = "$refSeqFile.$param{longWord}.db";
...
sub mapReadMrnaSeq_bowtie {
    ...
    $bowtieCommand .= " $param{refSeqPath}/$param{refSeq}/$refSeq"; #refSeq = refMrna_$param{refSeq}
...
sub enforceMaxHitsMrnaSeqRead {
    ...
    while (!$prevPairID){ #since many rRNA reads do not return a pairID
        my $line = <$mapFileH>; 
        $line or die "never got valid pairID from map file $outMapFiles{$read}";
...
sub getLine_pass_mRna { #modified to get corrected chrom, position and strand info from exons table
    ...
    $in[$gffFields{chromosome}] or return undef;
...
sub getLine_bowtie_mRna { #modified to get corrected chrom, position and strand info from exons table
    ...
    $in[$bowtieFields{chromosome}] or return undef;
...
sub fixName1Mrna {
    my ($name1) = @_;
    #this sub removes the '.#' designation added at the end of refGene sequence downloads
    #which are NOT present in the name1 field of the refGene table itself
    $$name1 =~ m/(.*)\.\d*$/ and $$name1 = $1; 
}
