...
sub parseFragments{
    ...
    $param{parseFragmentsOnly} and return;
...
sub getNDisc{
    my ($discs) = @_;  #easier way to do this is int(length($discs)/4 + 0.5)
    if ($discs){
        my $l = length($discs);
        my $n = int($l / 4);
        if ($l % 4){$n++}
...
sub commitRow{
    ...
    print $fragsFileH join(",",
        ...
        $$rowRef{CHROMOSOME1}, $$rowRef{POSITION1}, $$rowRef{LENGTH1}, $$rowRef{STRAND1}, $$rowRef{DISCREPANCIES1}, $$rowRef{NHITS1},
        $$rowRef{CHROMOSOME2}, $$rowRef{POSITION2}, $$rowRef{LENGTH2}, $$rowRef{STRAND2}, $$rowRef{DISCREPANCIES2}, $$rowRef{NHITS2},
...
sub purgeDuplicateFragments{
    ...
    $param{keepDups} and return;
    $inputFragsTable or $inputFragsTable = $fragsTable; 
    status("  purging any remaining duplicate Fragments\n");    

    #When more than one remaining fragment maps to the exact same pair of chromosome coordinates,
    #there must be a different number of discrepancies between those mappings.
    #In that situation, keep only those fragments at those coordinates
    #with the least number of discrepancies
    my $posID = " CHROMOSOME1 || 'x' || POSITION1 || 'x' || LENGTH1 || 'x' || STRAND1 || 'x' || 
                  CHROMOSOME2 || 'x' || POSITION2 || 'x' || LENGTH2 || 'x' || STRAND2 ";
    my $nDiscs = " (Round(Length(DISCREPANCIES1)/4) + Round(Length(DISCREPANCIES2)/4)) ";
    my $nDiscsSQL = " SELECT f.*, $posID posID, $nDiscs nDiscs FROM $inputFragsTable f ";
    my $minNDiscsSQL = " SELECT posID, Min(nDiscs) minNDiscs FROM ($nDiscsSQL) GROUP BY posID ";
    my $sql = "SELECT FRAGMENTID, FRAGMENTTYPE,
                      CHROMOSOME1, POSITION1, LENGTH1, STRAND1, DISCREPANCIES1, NHITS1,
                      CHROMOSOME2, POSITION2, LENGTH2, STRAND2, DISCREPANCIES2, NHITS2,
                      FRAGMENTSIZE, PAIRID, NFRAGS,
                      EVENTSIZE, STDEVNORMAL, ENDTOLERANCE, 
                      NSETSFRAG, NSETSPAIR
               FROM ($nDiscsSQL) f, ($minNDiscsSQL) md 
               WHERE f.posID = md.posID
                 AND f.nDiscs = md.minNDiscs";
    ...
    ##ADDED NHITS, but this is likely problematic!!  Switched to above
...
#                  CHROMOSOME1, POSITION1, LENGTH1, STRAND1, NHITS1,
#                  CHROMOSOME2, POSITION2, LENGTH2, STRAND2, NHITS2,
