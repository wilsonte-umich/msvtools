...
sub findTSS { 
    ...
    #  although HITS table does carry strand information
    #  TSS are determined WITHOUT respect to strand
    ...
    foreach my $tssThreshold(@tssThresholds){ 
        ...
        foreach my $binSize(@binSizes){ 
            ...
            foreach my $chrom(1..nChrom()){
                status(" $chrom");
                #this is where strand tracking would have to be added, if desired
