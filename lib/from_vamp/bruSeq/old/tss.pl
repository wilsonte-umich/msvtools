...
defined $param{tssThresholds} or $param{tssThresholds} = '0.65'; 
...
sub findTSS { 
    ...
    foreach my $tssThreshold(@tssThresholds){ 
        ...
        foreach my $binSize(@binSizes){ 
            ...
            #minBinHits = genome average hit density; could expose to user
            my $minBinHits = int((($uvHitCount*$uvScalar)/$genomeSize)*$binSize);
            ...
            foreach my $chrom(1..nChrom()){
                ...
                my $hitFracSQL = "SELECT bin, uCount/(uCount + nCount) fracUV
                                  FROM ($joinCountSQL)  
                                  WHERE (uCount > $minBinHits OR nCount > $minBinHits)
                                  ORDER BY bin";                                  
